...
module soc_fpga_tb_v1;
	...
	`ifdef SIM_VCS
		initial begin
			if ($test$plusargs("dump_on")) begin
				$display("Waveform Dumping Enabled (VCS/FSDB)");
	...
	// -------------------------
	// NO SV dumpers needed
	// (FST handled in sim_main.cpp)
	// -------------------------
	`ifdef SIM_VERILATOR
		initial begin
			if ($test$plusargs("dump_on"))
				$display("Waveform Dumping Enabled (Verilator/FST via C++)");
			else
				$display("Wave Dumps disabled!");
		end
	`endif
	...
	// -------------------------
	// Vivado uses WDB (TCL handles it)
	// -------------------------
	`ifdef SIM_VIVADO
		initial begin
			if ($test$plusargs("dump_on"))
				$display("Vivado WDB dumping handled by TCL scripts.");
			else
				$display("Wave Dumps disabled!");
		end
	`endif
	...
	// -------------------------
	// Default uses VCD fallback
	// -------------------------
	`ifdef SIM_DEFAULT	
		initial begin
			if ($test$plusargs("dump_on")) begin
				$display("Waveform Dumping Enabled (VCD fallback)");
